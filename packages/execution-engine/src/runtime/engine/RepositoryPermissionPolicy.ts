...
export function formatCrossRepoApprovalMessage(
  ...
  return `This request targets ${targetRepo}, but the selected repo is ${currentRepo}. Cross-repo access requires explicit approval before I can continue. Reply with \`approve cross-repo ${targetRepo} for 15m\` to continue.`;
}
