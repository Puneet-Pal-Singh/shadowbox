...
describe('CostCalculator', () => {
  ...
  describe('calculateTokenCost', () => {
    it('should calculate GPT-4o token cost', async () => {
      ...
      const cost = await calculator.calculateTokenCost('gpt-4o', 'openai', 1000, 1000)
    ...
    it('should calculate GPT-3.5-turbo token cost', async () => {
      ...
      const cost = await calculator.calculateTokenCost('gpt-3.5-turbo', 'openai', 1000, 1000)
    ...
    it('should handle fractional tokens', async () => {
      ...
      const cost = await calculator.calculateTokenCost('gpt-4o', 'openai', 500, 250)
      expect(cost).toBeCloseTo(0.00625, 5)
    })

    it('should handle zero output tokens', async () => {
      // Only input tokens
      const cost = await calculator.calculateTokenCost('gpt-4o', 'openai', 1000, 0)
      expect(cost).toBe(0.005)
    })

    it('should handle free models', async () => {
      // Groq llama3-70b is free
      const cost = await calculator.calculateTokenCost('llama3-70b', 'groq', 1000, 1000)
      expect(cost).toBe(0)
    })

    it('should throw for unknown model', async () => {
      await expect(
        calculator.calculateTokenCost('unknown-model-xyz', 'openai', 1000, 1000)
      ).rejects.toThrow(/Model not found/)
    })

    it('should throw for unknown provider', async () => {
      await expect(
        calculator.calculateTokenCost('gpt-4o', 'unknown-provider', 1000, 1000)
  ...
  describe('cost comparisons', () => {
    it('should show GPT-4o is more expensive than GPT-3.5-turbo', async () => {
      const gpt4Cost = await calculator.calculateTokenCost('gpt-4o', 'openai', 1000, 1000)
    ...
    it('should show Claude Opus is more expensive than Claude Sonnet', async () => {
      const opusCost = await calculator.calculateTokenCost('claude-3-opus', 'anthropic', 1000, 1000)
    ...
    it('should show free models cost $0', async () => {
      const groqCost = await calculator.calculateTokenCost('llama3-70b', 'groq', 10000, 10000)
      const llamaCost = await calculator.calculateTokenCost('llama2', 'ollama', 10000, 10000)
    ...
    it('should show output tokens cost more for most models', async () => {
      ...
      const inputOnlyCost = await calculator.calculateTokenCost('gpt-4o', 'openai', 1000, 0)
      const outputOnlyCost = await calculator.calculateTokenCost('gpt-4o', 'openai', 0, 1000)
